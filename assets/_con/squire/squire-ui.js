...
SquireUI = function(options) {
  if (typeof options.buildPath == "undefined") {
    options.buildPath = 'assets/_con/squire/';
  }
  // Create instance of iFrame
  var container, editor;
  if (options.replace) {
    container = $(options.replace).parent();
    options.value = options.value || $(options.replace).val();
  ...
  iframe.className += 'squire-con-iframe';
  ...
  iframe.addEventListener('load', function() {
    ...
    // add default value from textarea
    iframe.contentWindow.editor.setHTML(options.value);
  });
